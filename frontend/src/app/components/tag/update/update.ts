...
export class Update implements OnInit {
  ...
  loadTag(): void {
    this.loading = true;
    this.tagService.getTagById(this.categoryId).subscribe({
      next: (tag) => {
      ...
      error: (error) => {
        ...
        this.messageService.add({
          severity: 'error',
          summary: 'Error',
          detail: 'Error al cargar la etiqueta'
        });
        this.loading = false;
      }
    });
   ...
   submit(): void {
    if (this.form.valid) {
      ...
      this.tagService.updateTag(this.categoryId, tagData).subscribe({
        next: (response) => {
          this.messageService.add({
            severity: 'success',
            summary: 'Éxito',
        ...
        error: (error) => {
          ...
          this.messageService.add({
            severity: 'error',
            summary: 'Error',
            detail: 'Error al actualizar la etiqueta'
          });
          this.loading = false;
        }
      });
